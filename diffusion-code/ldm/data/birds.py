...
class FolderData(Dataset):
    def __init__(self,
        ...
        image_transforms.extend([transforms.ToTensor(),
                                 transforms.Lambda(lambda x: rearrange(x * 2. - 1., 'c h w -> h w c'))]) #changes order and scales to [-1,1]
    ...
    def __getitem__(self, index):
        data = {}
        if self.captions is not None:
            chosen = list(self.captions.keys())[index] #key in dict [this is filename.jpg]
            caption = self.captions.get(chosen, None) #list of 10 captions
            # randomly choose one caption from the list caption
